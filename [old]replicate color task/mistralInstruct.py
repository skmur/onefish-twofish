...
def getSample(context, prompt, temp, tokenizer, model, device):
    ...
    output = getOutput(messages, temp, tokenizer, model, device)
...
tokenizer = transformers.AutoTokenizer.from_pretrained("mistralai/Mistral-7B-Instruct-v0.1", cache_dir=lab_storage_dir)
model = transformers.AutoModelForCausalLM.from_pretrained("mistralai/Mistral-7B-Instruct-v0.1", cache_dir=lab_storage_dir, device_map="auto")
print("Model loaded...")
