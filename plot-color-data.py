...
def computeStats(df, metric):
    stats = df.groupby(['model_name', 'temperature', 'word', 'prompt', 'variance', 'entropy', 'imageability', 'concreteness'])['deltaE'].agg(['mean', 'count', 'std'])
...
def getPopulationDeltaE(df, words, model, temp, condition):
    ...
    df_output = pd.DataFrame(columns=['model_name', 'temperature', 'word', 'prompt', 'variance', 'entropy', 'imageability', 'concreteness', 'deltaE'])
...
conditions = ["none", "random", "nonsense", "identity"]
num_subjects = 150
temp = "default"
...
#         # add rss to df_rss
#         df_rss = pd.concat([df_rss, pd.DataFrame([[model, condition, rss, intercept]], columns=['model_name', 'prompt', 'rss', 'intercept'])])
...
# open df_rss
df_rss = pd.read_pickle(f"{output_dir}/rss_values-prompt=identity.pickle")
print(df_rss["prompt"].unique())
plt.figure(figsize=(10, 6))
sns.set_theme(style="whitegrid")
sns.barplot(x='model_name', y='rss', hue="prompt", data=df_rss)
# rotate x-axis labels
plt.xticks(rotation=45)
plt.title(f"Model RSS values for color task")
plt.tight_layout()
plt.savefig(f"./{figure_dir}/RSS.png")
plt.close()
