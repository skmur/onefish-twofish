...
def getOutput(messages, temp, tokenizer, pipe):
    output = pipe(
        messages,
        do_sample=True,
...
def getSample(context, prompt, temp, tokenizer, pipe):
    ...
    # if it doesn't find a hex code, try again up to 5 times and then return -1 for all values
...
def runTask(output_df, num_subjects, temp, condition, words, prompts_dict, model_name, task_version, tokenizer, pipe):
    ...
    for subject in range(num_subjects):
        ...
        for word in words:
            ...
            hex1, lab1, rgb1 = getSample(context, task_prompt, temp, tokenizer, pipe)
            hex2, lab2, rgb2 = getSample(context, task_prompt, temp, tokenizer, pipe)

            # for words where model refused to generate: if either value is -1, set deltae to -1 and skip this word
            if hex1 == -1 or hex2 == -1:
                deltae = -1
            else:
                # deltae
...
for task_version in task_versions:
    ...
    output_df = runTask(output_df, num_subjects, temp, condition, words, prompts_dict, model_name, task_version, tokenizer, pipe)
